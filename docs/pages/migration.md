...
Additionally, the accordion icon is now generated by a JavaScript component rather than being compiled into the CSS. To restore the icon, simply add a `span` element with the `uk-accordion-icon` attribute at the end of the accordion title. For example:
