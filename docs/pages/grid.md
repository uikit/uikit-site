...
To create the grid container, add the `.uk-grid` class to a `<div>` element. Add child `<div>` elements to create the cells. By default, all grid cells are stacked. To place them side by side, add one of the classes from the [Width component](width.md). Using `.uk-child-width-expand` will automatically apply equal width to items, regardless of how many there are.

**Note** A regular grid only needs the `.uk-grid` class and works without UIkit's JavaScript. Add the `uk-grid` attribute when using JavaScript features like the [Divider modifier](#divider-modifier), [Masonry](#masonry) or [Parallax](#parallax).
...
<div class="uk-grid uk-grid-medium uk-child-width-expand@s uk-text-center">
    <div>
        <div class="uk-card uk-card-default uk-card-body">Item</div>
    </div>
    <div>
...
<div class="uk-grid uk-grid-large uk-child-width-expand@s uk-text-center">
    <div>
        <div class="uk-card uk-card-default uk-card-body">Item</div>
    </div>
    <div>
...
<div class="uk-grid uk-grid-collapse uk-child-width-expand@s uk-text-center uk-margin-large-top">
    <div>
...
```html
<div class="uk-grid uk-grid-column-small uk-grid-row-large">…</div>
```

```example
<div class="uk-grid uk-grid-column-small uk-grid-row-large uk-child-width-1-3@s uk-text-center">
    <div>
        <div class="uk-card uk-card-default uk-card-body">Item</div>
    </div>
    <div>
...
```example
<div class="uk-grid uk-child-width-1-2 uk-text-center">
    <div>
        <div class="uk-card uk-card-default uk-card-body">Item</div>
    </div>
    <div>
...
Add the `.uk-grid-divider` class and the `uk-grid` attribute to separate grid cells with lines. This class can be combined with the gap modifiers. As soon as the grid stacks, the divider lines will become horizontal.

```html
<div class="uk-grid uk-grid-divider" uk-grid>…</div>
```
...
```html
<div class="uk-grid uk-grid-match">…</div>
```
...
```example
<div class="uk-grid uk-child-width-expand@s uk-text-center" uk-height-match="target: > div > .uk-card">
    <div>
        <div class="uk-card uk-card-default uk-card-body">Item</div>
     </div>
    <div>
...
```html
<div class="uk-grid uk-child-width-1-2@s uk-child-width-1-3@m">…</div>
```

```example
<div class="uk-grid uk-child-width-1-2@s uk-child-width-1-3@m uk-text-center">
    <div>
        <div class="uk-card uk-card-default uk-card-body">Item</div>
    </div>
    <div>
...
```example
<div class="uk-grid uk-grid-small uk-child-width-1-4@s uk-flex-center uk-text-center">
    <div>
...
If grid cells have different heights, add the `uk-grid` attribute and set the `masonry` option to either `pack` or `next` to create a layout free of gaps.
...
```html
<div class="uk-grid" uk-grid="masonry: pack">…</div>
```

```example
<div class="uk-grid uk-child-width-1-2@s uk-child-width-1-3@m" uk-grid="masonry: pack">
    <div>
...
To move single columns of a grid at different speeds while scrolling, add the `uk-grid` attribute and set `parallax: NUMBER`. The number sets the parallax translation in pixels.

```html
<div class="uk-grid" uk-grid="parallax: 150">…</div>
```

```example
<div class="uk-grid uk-child-width-1-2@s uk-child-width-1-3@m uk-child-width-1-4@l uk-text-center" uk-grid="parallax: 150">
    <div>
        <div class="uk-card uk-card-default uk-card-body">Item</div>
    </div>
    <div>
...
```html
<div class="uk-grid" uk-grid="parallax: 150; parallax-start: 100%; parallax-end: 100%;">…</div>
```
...
```html
<div class="uk-grid" uk-grid="parallax: 0; parallax-justify: true; masonry: pack">…</div>
```

```example
<div class="uk-grid uk-child-width-1-2@s uk-child-width-1-3@m" uk-grid="parallax: 0; parallax-justify: true; masonry: pack">
    <div>
        <div class="uk-card uk-card-default uk-flex uk-flex-center uk-flex-middle" style="height: 100px">Item 1</div>
    </div>
    <div>
...
Any of these options can be applied to the `uk-grid` component attribute. Separate multiple options with a semicolon. [Learn more](javascript.md#component-configuration)
