...
The Dropdown component is aim-aware. This means the dropdown stays opened as long as the mouse pointer moves to the dropdown. An additional delay ensures that the dropdown stays open even if the mouse pointer is moved shortly in another direction. A dropdown is closed immediately when another dropdown is opened.
