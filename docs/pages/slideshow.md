...
Autoplay stops when any element in the Slideshow component receives focus. The tab navigation can be accessed through keyboard using the following keys.
...
- The <kbd>left/right arrow</kbd> or <kbd>up/down arrow</kbd> keys, depending on the orientation, navigate through the tabs. The corresponding slide will get active automatically. If the focus is on the last tab, it move to the first tab. 
